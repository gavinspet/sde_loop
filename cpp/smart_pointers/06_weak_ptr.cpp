...
public:
  ...
  shrdptr<T>
  lock() const // create a shrdptr if the shared object is still valid
  {
    if (expired()) {
      return shrdptr<T>();
    }
...
int main() {
  ...
  std::cout << "wp1 expired: " << wp1.expired()
            << std::endl; // Output: wp1 expired: 0 (false)
  ...
  p1 = nullptr;
  std::cout << "p1 use count after reset: " << p1.use_count()
            << std::endl; // Output: 0
  std::cout << "wp1 expired: " << wp1.expired()
  ...
  if (auto sp = wp1.lock()) {
    ...
    std::cout << "weak pointer expired"
              << std::endl; // Output: weak pointer expired
  }
